...
#wall{
	...
	height:fit-content;
	width:100%;
	background-image: url("./images/uMan.svg");
...
<body style="margin:1%;background-color:#273239;">
	...
	<!-- Hidden Input to check  login creation-->
	...
	<div id="delete" class="popup">
		<div class="popup-content">
			<form action="admin_delete_login" method="post">
				<h5>
	...
	<div id="update" class="popup">
		<div class="popup-content">
			<form action="admin_update_login" method="post" style="text-align:left;">
				
				<div style="display:flex;flex-direction:row;justify-content:center;align-items:center;">
				<img src=""
					alt="Profile Image" width="300" height="300"
					 id="proImage" style="border-radius:50%;"></div>
					
				<br><br>
				<input type="hidden" id="userid" name="userid">
			
				<label style="font-weight:600;font-size:2vh;color:#4f52ff">User Name</label><br>
                <input type="text"  required  name="username" style="font-weight:600;" id="username"><br /><br>

				<label style="font-weight:600;font-size:2vh;color:#4f52ff">Email</label><br>
                <input type="email"  required  name="email" style="font-weight:600;" id="email"><br /><br>

                <input type="hidden"  required  name="password" style="font-weight:600;" id="password">
                
                
                <label style="font-weight:600;font-size:2vh;color:#4f52ff">Role</label><br>
                <select name="role" style="font-weight:600;">
                	<option value="student" style="font-weight:600;">Student</option>
                	<option value="teacher" style="font-weight:600;">Teacher</option>
                	<option value="admin"style="font-weight:600;">Admin</option>
                </select>
	...
	<div class="dashboard" style="displaye: flex; flex-direction: row;background-color:white;">
		<div class="panel" style="display: flex; flex-direction: column; width: 20vw; padding: 2%; height: fit-content; background-color: #f4f4f5;border-top-left-radius: 0%;border-top-right-radius: 80%;border-bottom-left-radius: 0%;border-bottom-right-radius: 80%">
			...
			<div class="column" style="padding:3%">
				<div class="col">
					<div class="collapse multi-collapse login-collapse show"
						id="login" onclick='redirectTo("http://localhost:8090/TestingPROJECT/PageLoadUsersManagement")' style="padding:2%;	background-color:#4f52ff;color:white;cursor:pointer;">
						<h6
							>Manage
							Logins</h6>
					</div>
	...
	<div style="display:flex;flex-direction:column;width:85vw;" id = "wall">
		...
		<div class="dataTable">
			<table id="tblDetail"
				style="max-width: 81.5%; box-shadow: 0 4px 8px #7f888e;background-color: rgba(255, 255, 255, 0.8);">
				<tr id="headlines">
					<th>ID</th>
					<th>User Name</th>
					<th>Email</th>
				...
				<c:forEach var="user" items="${Allusers}">
					<tr>
						...
						<td>${user.getEmail()}</td>
						<td>${user.getRole()}</td>
						<td><img src ="data:image/png;base64,${user.getImage()}" alt="No Images" height="50" width ="50" style="border-radius:25px;"></td>
						<td><button id="altBtn" onclick ="showPopupUpdate('${user.getId()}','${user.getUsername()}','${user.getEmail()}','${user.getPassword()}','${user.getImage()}')">Update</button></td>
						<td><button id="altBtn" onclick="showPopupDelete('${user.getId()}')">Delete</button></td>
					</tr>
				</c:forEach>
			</table>
		</div>
...
<script>
	...
	window.addEventListener("load", () => {
	  ...
	  loader.addEventListener("transitionend", () => {
		...
		//Show update box
		function showPopupUpdate(userid,username,email,password,image) {
			document.getElementById("userid").value = userid;
			document.getElementById("username").value = username;
			document.getElementById("email").value = email;
			document.getElementById("password").value = password;
			document.getElementById("proImage").src = 'data:image/png;base64,' + image;
