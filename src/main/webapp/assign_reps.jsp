...
#wall{
	height:90vh;
	width:100%;
	background-image: url("./images/repAss.svg");
...
<body style="margin:1%">
	...
	<div id="loginSuccessBox" class="popup">
		<div class="popup-content" style="width:25vw;height:30vh">
				<h3 style="color:#4f52ff;">Assignation Successful!</h3><br>
	...
	color:white;
	cursor:pointer;" onclick='redirectTo("http://localhost:8090/TestingPROJECT/assign_class_rep_loader")'>
						<h6
							>Assign
							representatives</h6>
					</div>
	...
	<div style="display:flex;flex-direction:column;width:85vw;" id = "wall">
		<h1 style="color:#263339;padding:0%;margin-left:2%;">Assign Class Representative</h1>
	       <form action="assign_class_rep" method="get" style="margin:2%;">
                ...
                <button type="submit" id="altBtn">Search</button>
...
<div class="dataTable">
    ...
    <!-- Check if SelectedClass is empty -->
    <c:if test="${empty classOnSelect}">
        <h1 style="color:#263339;padding:0%;font-size:1.8vh;margin-left:2%;">No class selected.</h1>
    </c:if>
    
     <c:if test="${not empty classOnSelect}">
        <h1 style="color:#263339;padding:0%;font-size:1.8vh;margin-left:2%;">Selected ${classOnSelect}</h1>
    </c:if>
    


    <c:if test="${not empty SelectedClass and not empty Allstudents}">
        <table id="tblDetail" style="max-width: 50%; box-shadow: 0 4px 8px rgba(79, 82, 255, 0.5);background-color: rgba(255, 255, 255, 0.97);">
            <tr id="headlines">
                <th>ID</th>
                <th>First Name</th>
                <th>Last Name</th>
            ...
            <c:forEach var="student" items="${Allstudents}">
                <tr>
                    ...
                    <td><button id="altBtn" onclick="showPopupUpdate('${student.getId()}','${student.getFirstName()}')">Assign</button></td>
                </tr>
            </c:forEach>
        </table>
    </c:if>
	...
	window.addEventListener("load", () => {
		...
		//Show approve box
		function showPopupUpdate(stid,stname) {
			document.getElementById("studentid").value = stid;
			document.getElementById("cMsg").innerHTML = "Assign " + stname + " as class representative ?";
