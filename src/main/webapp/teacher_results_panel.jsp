...
<body style="margin:1%;background-color:#273239;">
	...
	<div id="delete" class="popup">
		<div class="popup-content">
			<form action="teacher_result_delete" method="post">
				<h5>
					Are you sure you want to<br> delete this result?
				</h5>
				
				<input type="hidden" id="resid" value="" name="resid">
	...
	<div class="header" style="padding:1%;background-color:white;border-radius:71% 79% 0% 34% / 100% 100% 10% 0% ">
		...
		<div
			...
			<input type="text" id="searchInput" onkeyup="filterTable()"
				placeholder="Search by Result ID" style="margin-rigt: 2%;font-weight:600">
		</div>
			...
			<div class="column" style="padding:3%">
				<div class="col" >
					<div class="collapse multi-collapse res-collapse show"
						id="res" onclick='redirectTo("http://localhost:8090/TestingPROJECT/teacher_result_panel_loader")' style="padding:3%;	background-color:#4f52ff;
	color:white;
	cursor:pointer;">
						<h6
							>Published
	...
	<div style="display:flex;flex-direction:column;width:85vw;" id ="wall">
			<c:if test="${empty AllResults}">
        		<h1 style="color:#263339;font-size:5vh;padding:0%;margin-left:4vh;">No results has been published yet.</h1>
    		</c:if>
	...
	<c:if test="${not empty AllResults}">
		<div class="dataTable">
			<table id="tblDetail"
				...
				<c:forEach var="res" items="${AllResults}">
					<tr>
						<td>${res.getResultDescription()}</td>
						<td>${res.getSubjectName()}</td>
						<td>${res.getClassName()}</td>
						<td>${res.getUploaderName()}</td>
						<td>${res.getReleasedDate()  }</td>
						<td>
							<button onclick="openPdfModal('${res.getSheet()}')" id="altBtn">View</button>
						</td>
						<td><button id="altBtn" onclick = "showPopupDelete('${res.getResultid()}')">Delete</button></td>	
					</tr>
				</c:forEach>
			</table>
		</div></c:if>
...
<script>
	...
	window.addEventListener("load", () => {
		...
		//Show delete box
		function showPopupDelete(resid) {
