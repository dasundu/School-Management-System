...
<body style="margin:1%;background-color:#273239;">
	...
	<div id="delete" class="popup">
		<div class="popup-content">
			<form action="teacher_delete_assignment" method="post">
				<h5>
					Are you sure you want to<br> delete this assignment?
				</h5>
				<input type="hidden" id="assIdSpan" value="" name="assId">
			...
			<div class="column" style="padding:3%">
				<div class="col">
					<div class="collapse multi-collapse ass-collapse show"
						id="ass" onclick='redirectTo("http://localhost:8090/TestingPROJECT/teacher_announced_assignments_loader")' style="padding:3%;	background-color:#4f52ff;
	color:white;
	cursor:pointer;">
						<h6
							>Announced
...
<div style="display:flex;flex-direction:column;width:70vw;" id = "wall">
		<div class="dataTable">
			<c:if test="${empty AllAssignments}">
				<h1 style="color:#263339;font-size:5vh;padding:0%;margin-left:4vh;">No assignments announced.</h1>
			</c:if>
			<c:if test="${not empty AllAssignments}">
			<table id="tblDetail"
				style="max-width: 60%; box-shadow: 0 4px 8px #263339;background-color: rgba(255, 255, 255, 0.9);">
				<tr id="headlines">
					<th>Assignment Id</th>
					<th>Title</th>
					<th>Subject Id</th>
				...
				<c:forEach var="ass" items="${AllAssignments}">
					<tr>
						<td>${ass.getAssignmentId()}</td>
						<td>${ass.getTitle()}</td>
						<td>${ass.getSubjectid()}</td>
						<td>${ass.getClassid()}</td>
						<td>
							<button onclick="openPdfModal('${ass.getPdfFile()}')" id="altBtn">View PDF</button>
						</td>
						<td><button id="altBtn" onclick = "showPopupDelete('${ass.getAssignmentId()}')">Delete</button></td>
					</tr>
				</c:forEach>
			</table></c:if>
		</div></div>
...
<script>
	...
	window.addEventListener("load", () => {
		...
		//Show delete box
		function showPopupDelete(assIdSpan) {
			// Set the user ID in the popup content
			document.getElementById("assIdSpan").value =assIdSpan;
