...
<body style="margin:1%;background-color:#273239;">
	...
	<div id="loginSuccessBox" class="popup">
		<div class="popup-content" style="width:25vw;height:30vh">
				<h3 style="color:#4f52ff;">Class Update Successful!</h3><br>
	...
	<div id="delete" class="popup">
		<div class="popup-content">
			<form action="admin_delete_class" method="post">
				<h5>
					Are you sure you want to<br> delete this class?
				</h5>
				<input type="hidden" id="userIdSpan" value="" name="classid">
	...
	<div id="update" class="popup">
		<div class="popup-content">
		<form action="admin_update_class" method="post" style="margin:5%;width:20vw;text-align:left;">
			
				<input type="hidden" value="" id="classid" name="ClassId">
				
				<label for="subjectname">Class Name:</label><br>
			 	<input type="text" name="classname" value="" id="cname"><br> <br>
				
				<label for="description">Room Number:</label><br> 
				<input type="text" name="roomnumber" value="" id="rnumber"><br><br>
			
			<div style="display:flex;flex-direction:column;justify-content:center;align-items:center;">
			<button type="submit" id="altBtn" style="width:8vw">Update</button>
			<button id="altBtn" style="width:8vw" onclick="closePopupUpdate()">Cancel</button>
			</div>
	...
	<div class="header" style="padding:1%;background-color:white;border-radius:71% 79% 0% 34% / 100% 100% 10% 0% ">
		...
		<div
			...
			<input type="text" id="searchInput" onkeyup="filterTable()"
				placeholder="Search by Class ID" style="margin-rigt: 2%;font-weight:600">
		</div>
	...
	color:white;
	cursor:pointer;" onclick='redirectTo("http://localhost:8090/TestingPROJECT/admin_classes_loader")'>
						<h6
							>Manage
							Classes</h6>
					</div>
	...
	<div style="display:flex;flex-direction:column;width:85vw;" id ="wall">
		...
		<div class="dataTable">
			<table id="tblDetail"
				style="max-width: 81.5%; box-shadow: 0 4px 8px rgba(79, 82, 255, 0.5);background-color: rgba(255, 255, 255, 0.9);">
				<tr id="headlines">
					<th>Class Id</th>
					<th>Class Name</th>
					<th>Room Number</th>
				...
				<c:forEach var="classr" items="${Classes}">
					<tr>
						<td>${classr.getClassid()}</td>
						<td>${classr.getClassName()}</td>
						<td>${classr.getRoomNum() }</td>
						<td>${classr.getAssignedTeacher()}</td>
						<td>${classr.getRepresentative() }</td>
						<td>${classr.getNumberOfStudents()}</td>
						<td><button id="altBtn" onclick="showPopupUpdate('${classr.getClassName()}','${classr.getRoomNum() }','${classr.getClassid()}')">Update</button></td>
						<td><button id="altBtn" onclick="showPopupDelete('${classr.getClassid()}')">Delete</button></td>
					</tr>
				</c:forEach>
			</table>
		</div>
...
<a href="#" class="back-to-top">
	...
	window.addEventListener("load", () => {
		...
		//Show delete box
		function showPopupDelete(classid) {
			// Set the user ID in the popup content
			document.getElementById("userIdSpan").value = classid;
		...
		//Show update box
		function showPopupUpdate(classname,classnumber,classid) {
			document.getElementById("cname").value = classname;
			document.getElementById("rnumber").value = classnumber;
