...
#wall{
	...
	height:fit-content;
	width:100%;
	background-image: url("./images/assSub.svg");
...
<body style="margin:1%;background-color:#273239;">
	...
	<div id="update" class="popup">
		<div class="popup-content">
			<form action="teacher_assignment_evaluate" method="post">
				<h5>
					Assignment Evaluation
				</h5>
				<input type ="number" name="marks" value="marks"><br>
				<input type="hidden" id="submitid" value="" name="submitid"><br>
				
				<button id="yesBtn" type="submit">Evaluate</button>
				<button id="noBtn" onclick="closePopupUpdate(event)">Cancel</button>
			</form>
		</div>
	</div>
	...
	<div class="header" style="padding:1%;background-color:white;border-radius:71% 79% 0% 34% / 100% 100% 10% 0% ">
		...
		<div
			...
			<input type="text" id="searchInput" onkeyup="filterTable()"
				placeholder="Search by Assignment ID" style="margin-rigt: 2%;font-weight:600">
		</div>
	...
	<div class="dashboard" style="displaye: flex; flex-direction: row;background-color:white;">
		<div class="panel" style="display: flex; flex-direction: column; width: 20vw; padding: 2%; height: fit-content; background-color: #f4f4f5;border-top-left-radius: 0%;border-top-right-radius: 80%;border-bottom-left-radius: 0%;border-bottom-right-radius: 80%">
			...
			<div class="column" style="padding:3%">
				...
				<div class="col">
					<div class="collapse multi-collapse ass-collapse show"
						id="ass" onclick='redirectTo("http://localhost:8090/TestingPROJECT/teacher_manage_submitted_assignment_info_loader")' style="padding:3%;	background-color:#4f52ff;
	color:white;
	cursor:pointer;">
						<h6
							>Assignment
		...
		<div style="display:flex;flex-direction:column;width:70vw;" id="wall">
			...
			<div class="dataTable">
			<c:if test="${empty AllSubmittedAssignments}">
				<h1 style="color:#263339;font-size:5vh;padding:0%;margin-left:4vh;">No assignments Uploaded.</h1>
			</c:if>
			<c:if test="${not empty AllSubmittedAssignments}">
				<table id="tblDetail"
					style="max-width: 80%; box-shadow: 0 4px 8px #263339;background-color: rgba(255, 255, 255, 0.9);">
					<tr id="headlines">
						<th>Assignment</th>
						<th>Subject</th>
						<th>Student ID </th>
					...
					<c:forEach var="ass" items="${AllSubmittedAssignments}">
						<tr>
							<td>${ass.getAssignmentName()}</td>
							<td>${ass.getSubjectName() }
							<td>${ass.getStudentid()}</td>
							<td>${ass.getStudentName()}</td>
							<td>
								<button onclick="openPdfModal('${ass.getUploadedFile()}')"
									id="altBtn">View PDF</button>

							</td>
							<td><c:choose>
									<c:when test="${ass.getMarks() >= 0}">
            ${ass.getMarks()}
        </c:when>
									<c:otherwise>
            Not Evaluated
								...
								<td><button id="altBtn" onclick = "showPopupUpdate('${ass.getSubmitid()}')">Evaluate</button></td>
						</tr>
					</c:forEach>
				</table>
			</c:if>
	...
	<script>
		...
		//Show evaluate box
		function showPopupUpdate(submitid) {
