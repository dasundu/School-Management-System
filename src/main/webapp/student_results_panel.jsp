...
<body style="margin:1%;background-color:#273239;">
			...
			<div class="column" style="padding:3%">
				<div class="col"  onclick='redirectTo("http://localhost:8090/TestingPROJECT/student_results_info")'>
					<div class="collapse multi-collapse res-collapse show"
						id="res" style="padding:3%;	background-color:#4f52ff;
	color:white;
	cursor:pointer;">
						<h6
							>Published
	...
	<div style="display:flex;flex-direction:column;width:70vw;" id="wall">
		<c:if test="${empty AllResults}">
			<h1 style="color:#263339;font-size:5vh;padding:0%;margin-left:4vh;">Results not found.</H1>
		</c:if>
		...
		<c:if test="${not empty AllResults}">
			...
			<div style="display:flex;flex-flow:row wrap;justify-content:center;align-items:center;">
				...
				<div style="box-shadow: 0 4px 8px rgba(79, 82, 255, 0.5);width:17vw;height:17vh;padding:2%;margin:2%;background-color: rgba(255, 255, 255, 0.95);">
					
						<h4> ${res.getSubjectName()}  ${res.getClassName()}</h4>
						<p style="color:#4f52ff;font-weight:600">${res.getResultDescription()}</p>
						<p style="font-weight:600">${res.getReleasedDate()  }</p>
						<button onclick="openPdfModal('${res.getSheet()}')" style="text-decoration:underline;color:#4f52ff;font-weight:600;border:none;background-color:white;">Result Sheet</button>
