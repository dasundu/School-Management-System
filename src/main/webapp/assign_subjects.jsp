...
#wall{
	height:90vh;
	width:100%;
	background-image: url("./images/assSubTeacher.svg");
...
<body style="margin:1%;background-color:#273239;">
	...
	<div id="loginSuccessBox" class="popup">
		<div class="popup-content" style="width:25vw;height:30vh">
				<h3 style="color:#4f52ff;">Assignation Successfully!</h3><br>
	...
	color:white;
	cursor:pointer;" onclick='redirectTo("http://localhost:8090/TestingPROJECT/subject_assign_loader")'>
						<h6
							>Assign
							Teachers</h6>
					</div>
...
<form action="assignSubjectsServlet" method="post" style="margin:5%;box-shadow: 0 4px 8px rgba(79, 82, 255, 0.5);padding:3%;width:33vw;background-color: rgba(255, 255, 255, 0.9);">
<h1 style="color:#263339">Assign Teachers To Subjects</h1>

    <label for="subjectSelect">Select Subject:</label><br>
    <select name="subjectSelect">
        <c:forEach var="subject" items="${AllSubjects}">
            <option value="${subject.getSubjectId() }">${subject.getSubName()}</option>
        </c:forEach>
    </select><br><br>
    ...
    <select name="teacherSelect">
        <c:forEach var="teacher" items="${AllTeachers}">
            <option value="${teacher.getId()}">${teacher.getFirstName()} ${teacher.getLastName() } - ${teacher.getSpecialization() }</option>
        </c:forEach>
    </select><br><br><br>
	
	
	<div style="display:flex;flex-direction:column;justify-content:center;align-items:center;">
    <button type="submit" id="altBtn" style="width:10vw;">Assign</button></div>
</form>
	</div>
