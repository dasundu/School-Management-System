...
<body style="margin:1%;background-color:#273239;">
	...
	<div class="dashboard" style="displaye: flex; flex-direction: row;background-color:white;border-bottom-right-radius: 5%;">
		<div class="panel" style="display: flex; flex-direction: column; width: 20vw; padding: 2%; height: fit-content; background-color: #f4f4f5;border-top-left-radius: 0%;border-top-right-radius: 80%;border-bottom-left-radius: 0%;border-bottom-right-radius: 80%">
			...
			<div class="column" style="padding: 3%">
				<div class="col"
					...
					<div class="collapse multi-collapse ass-collapse show" id="ass"
						style="padding: 3%">
		...
		<div style="display:flex;flex-flow:column;width:70vw;" id = "wall">
			...
			<div class="dataTable" style="margin: 5%">
				...
				<c:if test="${not empty AllSubmittedAssignments}">
					<div style="display:flex;flex-flow:row wrap;justify-content:center;align-items:center;">
						...
						<c:forEach var="ass" items="${AllSubmittedAssignments}">
							<div style="box-shadow: 0 4px 8px #263339;width:25vw;height:fit-content;padding:2%;margin:2%;background-color: rgba(255, 255, 255, 0.96);">
							
								<h4>${ass.getAssignmentName()}</h4>
								<p style="color:#4f52ff;font-weight:600">Subject :${ass.getSubjectName() }</p>
								<p><button onclick="openPdfModal('${ass.getUploadedFile()}')" id="altBtn">Uploaded File</button></p>
								<c:choose>
									<c:when test="${ass.getMarks() >= 0}">
										<p style="font-weight:600">Marks - ${ass.getMarks()}</p>
									</c:when>
									<c:otherwise>
										<p style="font-weight:600">Marks  - Not Evaluated</p>
									</c:otherwise>
								</c:choose>
							</div>
						</c:forEach>
