...
<body style="margin:1%;background-color:#273239;">
	...
	color:white;
	cursor:pointer;" onclick='redirectTo("http://localhost:8090/TestingPROJECT/admin_assignment_loader")'>
						<h6
							>Manage
							Assignments</h6>
					</div>
	...
	<div style="display:flex;flex-direction:column;width:85vw;" id="wall">
		<div class="dataTable">
			<table id="tblDetail"
				style="max-width: 55%; box-shadow: 0 4px 8px #263339;background-color: rgba(255, 255, 255, 0.8);">
				<tr id="headlines">
					<th>Assignment Id</th>
					<th>Title</th>
					<th>Subject Id</th>
				...
				<c:forEach var="ass" items="${AllAssignments}">
					<tr>
						...
						<td>
							<button onclick="openPdfModal('${ass.getPdfFile()}')" id="altBtn">View
								PDF</button>
						</td>
						<td>${ass.getUploader()  }</td>
					</tr>
				</c:forEach>
			</table>
		</div></div>
...
<script>
    ...
    <script>
        // Add this script to hide the loader once the page is fully loaded
